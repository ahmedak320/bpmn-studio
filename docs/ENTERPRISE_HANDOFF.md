...
1. **Tarball.** Run `npm run build:lib`, then `npm pack` inside
   ...
   `orbitpm-epc-engine-0.1.0.tgz` in the enterprise repository as an ordinary
...
| `project`         | As `validate`, then writes `draft.json`, `model.aml.xml`, `findings.json`, and `narrative.md`. Still never boots jsdom — it stops one step before the canvas.                                                |
| `render`          | As `project` (structurally), then writes `process.svg`, `metadata.json`, `findings.json`, and `narrative.md`. The only command that boots the headless jsdom canvas, and only after a clean structural gate. |
...
`buildVerificationPackage(process: CanonicalProcessV1): VerificationPackageV1`
