...
another — see `duplicate-id` below).
...
### Cross-reference integrity (11 stable issue codes)
...
| `duplicate-id`                      | An id is reused across the 8 top-level entity arrays (`nodes`/`decisions`/`edges`/`roles`/`systems`/`informationObjects`/`controls`/`facts`).                                                         |
...
### The expansion table (verbatim — every row implemented in `projectToEpc.ts`)
...
| `sequence` / `handoff` edge                                                   | control-flow relation `CT_ACTIV_1` when source is `OT_EVT`→func, `CT_CRT_1` when func→event, `CT_LEADS_TO_1` into/out of rules (picked by endpoint types exactly as the connection-type table above); `handoff` edges carry `names` from the canonical edge if present                                                                                                                                                                                                       |
