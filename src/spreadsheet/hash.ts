/** Stable, non-cryptographic hash used only for signatures and deterministic IDs. */
...
/**
 * Browser-safe SHA-256 with no Node import. This keeps report checksums
 * deterministic in file:// builds and makes the checksum adapter optional.
 */
export function sha256Hex(input: Uint8Array): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + sum1 + choice + SHA256_K[index]! + schedule[index]!) >>> 0
