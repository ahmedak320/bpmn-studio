...
describe('handleIndexBuilderWorkerRequest', () => {
  ...
  it('never throws — a malformed request still resolves an error response', () => {
    ...
    const request = {
      ...
      requestId: 'req-2',
      models: null
    } as unknown as IndexBuilderWorkerRequest
    let response: IndexBuilderWorkerResponse | undefined
...
describe('buildDigestsInBrowser — fallback path (no Worker global, e.g. Node tests)', () => {
  it('resolves digests synchronously in-thread when Worker is unavailable', async () => {
    const digests = await buildDigestsInBrowser([
      buildSyntheticModel(),
      buildSecondSyntheticModel()
    ])
  ...
  it('rejects with AbortError when the signal is already aborted', async () => {
    ...
    controller.abort()
    await expect(
      buildDigestsInBrowser([buildSyntheticModel()], { signal: controller.signal })
...
class MockWorker implements WorkerLike {
  ...
  private readonly messageListeners: Array<
    (event: MessageEvent<IndexBuilderWorkerResponse>) => void
  > = []
  ...
  addEventListener(type: 'message' | 'error', listener: (event: never) => void): void {
    if (type === 'message')
      this.messageListeners.push(
        listener as (event: MessageEvent<IndexBuilderWorkerResponse>) => void
      )
  ...
  emitMessage(data: IndexBuilderWorkerResponse): void {
    for (const listener of [...this.messageListeners])
      listener({ data } as MessageEvent<IndexBuilderWorkerResponse>)
  }
}
...
describe('buildDigestsInBrowser — worker protocol via an injected mock worker', () => {
  ...
  it('posts a build request and resolves with the worker-reported digests', async () => {
    ...
    const mock: MockWorker = new MockWorker((request) => {
      mock.emitMessage({
        ...
        digests: [buildDigestOf(buildSyntheticModel())]
      })
    })
    const digests = await buildDigestsInBrowser([buildSyntheticModel()], {
      workerFactory: () => mock
    })
  ...
  it('rejects when the worker reports an error', async () => {
    ...
    await expect(
      buildDigestsInBrowser([buildSyntheticModel()], { workerFactory: () => mock })
