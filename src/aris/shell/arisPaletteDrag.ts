...
//    DOM dependency, exercised directly by src/aris/shell/arisPaletteDrag.test.ts
//    (which runs with environment: 'node', no jsdom — DOM code below is
//    intentionally untested there).
//  - installPaletteDrag, the DOM-wiring entry point the ARIS shell calls once per
 ...
 *  Modeler, so in practice it already exists by the time the shell calls
