...
describe('authoring operations (Section 11.4)', () => {
  ...
  it('creates another occurrence of an existing definition', () => {
    ...
    expect(findOccurrence(canvas.document, secondOccurrenceId)?.definitionId).toBe(
      first.definitionId
    )
    // Renaming the definition renames both shapes.
    canvas.authoring.renameDefinition(first.definitionId, 'Approve v2')
    expect(arisBusinessObject(shape(canvas, first.occurrenceId))).toMatchObject({
      name: 'Approve v2'
    })
    expect(arisBusinessObject(shape(canvas, secondOccurrenceId))).toMatchObject({
      name: 'Approve v2'
    })
  })
  ...
  it('moves, resizes and restyles an occurrence', () => {
    ...
    canvas.authoring.restyleOccurrence(created.occurrenceId, {
      fillColor: '#123456',
      strokeWidth: 3
    })
  ...
  it('reconnects a connection to a new target', () => {
    ...
    expect(
      canvas.document.connectionDefinitions.get(reconnected?.definitionId ?? '')
    ).toMatchObject({
      fromObjectDefinitionId: a.definitionId,
    ...
    canvas.undo()
    expect(
      canvas.document.models.get(canvas.activeModelId)?.connectionOccurrences[0].targetOccurrenceId
    ).toBe(b.occurrenceId)
  })
  ...
  it('adds and edits a lane', () => {
    harness = bootCanvas()
    const { canvas } = harness
    const laneId = canvas.authoring.addLane({
      name: 'Registry',
  ...
  it('adds and edits free text', () => {
    ...
    canvas.authoring.editFreeText(freeTextId, {
      text: 'Final',
      bounds: { x: 20, y: 20, width: 240, height: 60 }
    })
  ...
  it('adds, downloads and removes an attachment', () => {
    ...
    canvas.undo()
    expect(canvas.authoring.listAttachments(created.definitionId)).toHaveLength(1)
    expect(() => canvas.authoring.downloadAttachment(created.definitionId, 'nope')).toThrow(
      /No attachment/u
    )
  })
  ...
  it('requires a separate confirmation to delete an unused definition', () => {
    ...
    ).toThrow(/still has 1 occurrence/u)
    expect(canvas.document.objectDefinitions.has(created.definitionId)).toBe(true)
    ...
    expect(() =>
      canvas.authoring.confirmDeleteDefinition(created.definitionId, { confirmed: false })
  ...
  it('finds and selects an element by caption (search/select)', () => {
    ...
    const other = canvas.authoring.createObject({
      objectType: 'OT_EVT',
      name: 'Animal registered',
      position: { x: 0, y: 300 }
    })
